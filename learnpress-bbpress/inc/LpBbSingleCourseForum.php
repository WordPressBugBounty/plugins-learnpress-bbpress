...
class BbSingleCourseForumTemplate {
	use Singleton;
	...
	public function html_forum_author( int $forum_id, int $size = 50 ): string {
		...
		$user_model  = UserModel::find( $author_id );
		$author_url  = bbp_get_user_profile_url( $author_id );
		$avatar_html = UserTemplate::instance()->html_avatar(
			$user_model,
			array(
				'height' => $size,
				'width'  => $size,
	...
	public function html_forum_link( int $forum_id ): string {
		...
		$html       = sprintf( '<a class="lp-bbpress-forum-link lp-button" href="%1$s">%2$s</a>', $forum_link, __( 'Join Forum', 'learnpress-bbpress' ) );
		return $html;
	}
